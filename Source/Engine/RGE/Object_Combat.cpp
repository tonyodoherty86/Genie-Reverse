...
/**
 * @file    Engine\RGE\CombatObject.cpp
 * @author  Yvan Burrie
 * @date    2018/07/07
 * @version 1.0
 */
    ...
    float z)
{
    ...
    this->attack_timer = this->master_obj->speed_of_attack;
    this->capture_flag = 0;

    return true;
}

bool RGE_Combat_Object::setup(int infile, RGE_Game_World *gworld)
{
    memset(this->VUR_Ptrs, 0, sizeof(this->VUR_Ptrs));

    this->Unified_Map_Value = 0;

    RGE_Action_Object::setup(infile, gworld);

#if RGE_COMBAT_OBJECT_FORMATION_EXISTS
    if( save_game_version >= 9.05 ){
        rge_read(infile, &this->formation_id, 1);
        rge_read(infile, &this->formation_row, 1);
        rge_read(infile, &this->formation_col, 1);
    }
#endif

    rge_read(infile, &this->attack_timer, 4);

#if RGE_COMBAT_OBJECT_CAPTURE_FLAG_EXISTS
    if( save_game_version >= 2.01 ){
        rge_read(infile, &this->capture_flag, 1);
    }
#endif

#if RGE_COMBAT_OBJECT_VISIBILITY_INFO_EXISTS
    if( save_game_version >= 9.09 ){
        rge_read(infile, &this->multi_unified_point, 1);
        rge_read(infile, &this->large_radius, 1);
    }
#endif

#if RGE_COMBAT_OBJECT_ATTACK_COUNT_EXISTS
    if( save_game_version >= 10.02 ){
        rge_read(infile, &this->attack_count, 4);
    }
#endif
...
void RGE_Combat_Object::save(int outfile)
{
    RGE_Action_Object::save(outfile);

#if RGE_COMBAT_OBJECT_FORMATION_EXISTS
    if( save_game_version >= 9.05 ){
        rge_write(outfile, &this->formation_id, 1);
        rge_write(outfile, &this->formation_row, 1);
        rge_write(outfile, &this->formation_col, 1);
    }
#endif

    rge_write(outfile, &this->attack_timer, 4);

#if RGE_COMBAT_OBJECT_CAPTURE_FLAG_EXISTS
    if( save_game_version >= 2.01 ){
        rge_write(outfile, &this->capture_flag, 1);
    }
#endif

#if RGE_COMBAT_OBJECT_VISIBILITY_INFO_EXISTS
    if( save_game_version >= 9.09 ){
        rge_write(outfile, &this->multi_unified_point, 1);
        rge_write(outfile, &this->large_radius, 1);
    }
#endif

#if RGE_COMBAT_OBJECT_ATTACK_COUNT_EXISTS
    if( save_game_version >= 10.02 ){
        rge_write(outfile, &this->attack_count, 4);
    }
#endif
...
void RGE_Combat_Object::stop()
{
    ...
    if( this->unitAIValue ){
        if( actionFile ){
            fprintf(actionFile, aDCallStopobjec, this->id, aCMsdevWorkA_11, 171);
        }
        this->unitAIValue->stopObject(1);
    }
}

double RGE_Combat_Object::calc_attack_modifier(
    RGE_Static_Object *target)
{
    return 1.0;
}

char RGE_Combat_Object::area_attack(
    float x,
    float y,
    float z,
    RGE_Combat_Object *attacker,
    RGE_Static_Object *ignore_object)
{
    RGE_Combat_Object *v6; // esi@1
    double v7; // st7@1
    char result; // al@2
    signed short v9; // di@3
    int v10; // eax@3
    int v11; // ebp@14
    ...
    if( v7 > 0.0 )
    {
        ...
        if( (signed short)x0 < 0 )
            LOWORD(x0) = 0;
        if( (signed short)x1 > (signed short)(LOWORD(map->map_width) - 1) + 1 )
            LOWORD(x1) = map->map_width;
        if( v9 < 0 )
            v9 = 0;
        if( (signed short)v10 > (signed short)(LOWORD(map->map_height) - 1) + 1 )
            LOWORD(v10) = map->map_height;
        if( v9 < (signed short)v10 )
        {
            v26 = v9;
            v29 = (signed short)v10 - v9;
            do
            {
                if( (signed short)x0 < (signed short)x1 )
                {
                    v11 = (signed short)x0;
                    v28 = (signed short)x1 - (signed short)x0;
                    do
                    {
                            ...
                            if( (RGE_Combat_Object *)i->node != v6 )
                            {
                                    ...
                                    && v13 != ignore_object )
                                {
                                    ...
                                    if( r2 > v19 * v19 + v20 * v20
                                        && (signed short)(100 * debug_rand(aCMsdevWorkA_11, 250) / 0x7FFF) <= v6->master_obj[1].attribute_type_held[2] )
                                    {
                                        v6->vfptr->calc_attack_modifier((RGE_Static_Object *)v6, v13);
                                        v21 = v19;
                                        ((void (*)(RGE_Static_Object *, _DWORD, _DWORD, _DWORD, RGE_Player *, RGE_Combat_Object *))v13->vfptr->damage)(
                                            v13,
...
char RGE_Combat_Object::attack(RGE_Static_Object *target, RGE_Combat_Object *attacker)
{
    ...
    if( v7 < 0 )
    {
        ((void (*)(RGE_Combat_Object *, RGE_Static_Object *, RGE_Combat_Object *, _DWORD, _DWORD, _DWORD))v4->vfptr[1].remove_overlay_sprite)(
            v4,
            target,
            attacker,
            LODWORD(v4->world_x),
...
char RGE_Combat_Object::attack(float target_x, float target_y, float target_z, RGE_Combat_Object *attacker)
{
...
char RGE_Combat_Object::do_attack(RGE_Static_Object *target, RGE_Combat_Object *attacker, float x, float y, float z)
{
    ...
    if( target )
    {
        ...
        if( v9 <= 0.0 )
        {
            ...
            if( 100 * debug_rand(aCMsdevWorkA_11, 379) / 0x7FFF <= v7->master_obj[1].attribute_type_held[2] )
            {
                ...
                *(float *)&targeta = v9;
                ((void (*)(RGE_Static_Object *, _DWORD, _DWORD, RGE_Static_Object *, RGE_Player *, RGE_Combat_Object *))v6->vfptr->damage)(
                    v6,
...
void RGE_Combat_Object::damage(int weapon_num, RGE_Armor_Weapon_Info *damage, float attack_modifier, RGE_Player *attacking_player, RGE_Static_Object *attackingObject)
{
    RGE_Combat_Object *v7; // esi@1
...
double RGE_Combat_Object::calculateDamage(int weapon_num, RGE_Armor_Weapon_Info *damage, float attack_modifier, RGE_Player *attacking_player, RGE_Static_Object *attackingObject)
{
    RGE_Master_Static_Object *v6; // eax@1
    double v7; // st7@1
    short v8; // dx@1
...
bool RGE_Combat_Object::can_attack()
{
...
void RGE_Combat_Object::get_armor(short *cur_armor_in, short *orig_armor_in)
{
...
void RGE_Combat_Object::get_weapon(short *cur_weapon_in, short *orig_weapon_in)
{
    signed int v3; // edx@1
...
void RGE_Combat_Object::get_weapon_range(float *cur_weapon_range_in, float *orig_weapon_range_in)
{
    *cur_weapon_range_in = this->master_obj[1].construction_radius_x;
    *orig_weapon_range_in = *(float *)&this->master_obj[1].attribute_flag[0];
}

void RGE_Combat_Object::get_speed_of_attack(float *cur_speed_of_attack_in, float *orig_speed_of_attack_in)
{
    *cur_speed_of_attack_in = *(float *)&this->master_obj[1].movement_type;
    *orig_speed_of_attack_in = *(float *)&this->master_obj[1].combat_level;
}

void RGE_Combat_Object::release_being_worked_on(RGE_Static_Object *caller)
{
...
void RGE_Combat_Object::enter_obj(RGE_Static_Object *target)
{
    RGE_Combat_Object *v2; // esi@1
    int v3; // edi@2
...
bool RGE_Combat_Object::update()
{
    ...
    bool update = RGE_Action_Object::update();
    v6 = v1->id;
    if( this->id >= 0 &&
        ...
        v10 = unified_map_offsets[(unsigned int)this->world_y][(unsigned int)this->world_x];
        this->Unified_Map_Value = unified_map_offsets[(unsigned int)this->world_y][(unsigned int)this->world_x];
        if( v10 != v9 )
        {
            VisibleUnitManager->Update_Unit_Info(
                this->id,
                this->owner->id,
                v7,
                v8,
...
LABEL_13:
    ...
    if( this->unitAIValue &&
        this->owner->world->is_player_turn(this->owner->id) ){

        this->unitAIValue->update(v2, this->owner->world->get_accum_time_delta(this->owner->id));
    }

    return update;
}

void RGE_Combat_Object::copy_obj(RGE_Master_Static_Object *source2)
{
    RGE_Combat_Object *v2; // esi@1
    RGE_Sound *v3; // eax@1
    ...
    if( v3 )
        (*(void (**)(RGE_Combat_Object *, RGE_Sound *))&this->vfptr->gap4[52])(this, v3);
    RGE_Action_Object::copy_obj((RGE_Action_Object *)&v2->vfptr, source2);
}

//----- (00430850) --------------------------------------------------------
void RGE_Combat_Object::notify(int caller, int recipient, int mType, int p1, int p2, int p3)
{
...
//----- (004308C0) --------------------------------------------------------
RGE_Static_Object *RGE_Combat_Object::attack(int targetID, int unconditional)
{
    ...
    if( result )
    {
            ...
            && (v8 = RGE_Action_List::get_action(v3->actions), v8->vfptr->type(v8) != 7) )
        {
                ...
                || (v10 = RGE_Action_List::get_action(v3->actions), v10->vfptr->type(v10) != 9) )
            {
                ...
                (*(void (**)(RGE_Combat_Object *, int))&v3->vfptr[1].gap4[28])(v3, v15);
            }
            else
            {
                v11 = RGE_Action_List::get_action(v3->actions);
                ((void (*)(RGE_Action *, int, _DWORD, _DWORD, _DWORD))v11->vfptr->work)(
                    v11,
                    v5,
                    *(_DWORD *)(v5 + 56),
                    *(_DWORD *)(v5 + 60),
...
//----- (00430A90) --------------------------------------------------------
int RGE_Combat_Object::attack(float x, float y, float z, int unconditional)
{
    RGE_Combat_Object *v5; // esi@1
    RGE_Action *v6; // eax@3
    RGE_Action *v7; // eax@4
        ...
        && (v10 = RGE_Action_List::get_action(v5->actions), v10->vfptr->type(v10) != 7) )
    {
        ...
        (*(void (**)(RGE_Combat_Object *, int))&v5->vfptr[1].gap4[28])(v5, v13);
    }
    return 1;
}

//----- (00430C10) --------------------------------------------------------
int RGE_Combat_Object::moveTo(int targetID, int unconditional)
{
    return (*(int (__stdcall **)(int, signed int, int))&this->vfptr->gap154[4])(targetID, 1065353216, unconditional);
}

//----- (00430C30) --------------------------------------------------------
RGE_Static_Object *RGE_Combat_Object::moveTo(int targetID, float minRange, int unconditional)
{
    RGE_Combat_Object *v4; // esi@1
    RGE_Static_Object *result; // eax@1
    ...
    if( result )
    {
            ...
            || (v8 = RGE_Action_List::get_action(v4->actions), v8->vfptr->type(v8) != 1) )
        {
            ...
            (*(void (**)(RGE_Combat_Object *, int))&v4->vfptr[1].gap4[28])(v4, v13);
        }
        result = (RGE_Static_Object *)1;
    }
    return result;
...
//----- (00430D50) --------------------------------------------------------
int RGE_Combat_Object::moveTo(float x, float y, float z, float minRange, int unconditional)
{
    RGE_Combat_Object *v6; // esi@1
    RGE_Action *v7; // eax@3
    RGE_Action *v8; // eax@4
        ...
        || (v10 = RGE_Action_List::get_action(v6->actions), v10->vfptr->type(v10) != 1) )
    {
        if( RGE_Action_List::have_action(v6->actions) != 1
            || (v11 = RGE_Action_List::get_action(v6->actions),
                    ((int (*)(RGE_Action *, RGE_Combat_Object *, _DWORD, _DWORD, _DWORD))v11->vfptr->move_to)(
                        v11,
                        v6,
                        LODWORD(x),
                        LODWORD(y),
                        LODWORD(z)) != 1) )
        {
            ...
            (*(void (**)(RGE_Combat_Object *, int))&v6->vfptr[1].gap4[28])(v6, v16);
        }
    }
    return 1;
}
...
//----- (00430ED0) --------------------------------------------------------
RGE_Static_Object *RGE_Combat_Object::gather(int targetID, int unconditional)
{
    ...
    if( result )
    {
            ...
            || (v7 = RGE_Action_List::get_action(v3->actions), v7->vfptr->type(v7) != 5) )
        {
            ...
            {
                v10 = v3->vfptr;
                result = (RGE_Static_Object *)(*(int (**)(RGE_Combat_Object *, int, _DWORD, _DWORD, _DWORD))&v3->vfptr[1].gap4[36])(
                                                                                v3,
                                                                                v5,
                                                                                *(_DWORD *)(v5 + 56),
                                                                                *(_DWORD *)(v5 + 60),
                ...
                if( !v14 )
                    return 0;
                RGE_Action_List::delete_list(v3->actions);
                (*(void (**)(RGE_Combat_Object *, int))&v10[1].gap4[28])(v3, v14);
            }
            else
            {
                v9 = RGE_Action_List::get_action(v3->actions);
                ((void (*)(RGE_Action *, int, _DWORD, _DWORD, _DWORD))v9->vfptr->work)(
                    v9,
                    v5,
                    *(_DWORD *)(v5 + 56),
                    *(_DWORD *)(v5 + 60),
...
//----- (00431020) --------------------------------------------------------
int RGE_Combat_Object::explore(int tX, int tY, int unconditional)
{
    RGE_Combat_Object *v4; // esi@1
    RGE_Action *v5; // eax@3
    RGE_Action *v6; // eax@5
        ...
        || (v5 = RGE_Action_List::get_action(v4->actions), v5->vfptr->type(v5) != 4) )
    {
            ...
            || (v6 = RGE_Action_List::get_action(v4->actions), v6->vfptr->type(v6) != 4) )
        {
            ...
            if( !v14 )
                return 0;
            RGE_Action_List::delete_list(v4->actions);
            (*(void (**)(RGE_Combat_Object *, int))&v4->vfptr[1].gap4[28])(v4, v14);
        }
        else
        {
            ...
            y = (double)tY;
            x = (double)tX;
            ((void (*)(RGE_Action *, _DWORD, _DWORD, _DWORD, _DWORD))v7->vfptr->work)(
                v7,
                0,
                LODWORD(x),
                LODWORD(y),
...
//----- (00431150) --------------------------------------------------------
RGE_Static_Object *RGE_Combat_Object::enter(int targetID, int unconditional)
{
    RGE_Combat_Object *v3; // esi@1
    RGE_Static_Object *result; // eax@1
    int v5; // edi@1
    ...
    if( result )
    {
            ...
            || (v7 = RGE_Action_List::get_action(v3->actions), v7->vfptr->type(v7) != 3) )
        {
            ...
            if( !v12 )
                return 0;
            RGE_Action_List::delete_list(v3->actions);
            (*(void (**)(RGE_Combat_Object *, int))&v8[1].gap4[28])(v3, v12);
        }
        result = (RGE_Static_Object *)1;
    }
    return result;
...
//----- (00431250) --------------------------------------------------------
int RGE_Combat_Object::transport(float tX, float tY, float tZ, int unconditional)
{
    RGE_Combat_Object *v5; // esi@1
    RGE_Action *v6; // eax@2
    RGE_Action *v7; // eax@3
        ...
        || (v6 = RGE_Action_List::get_action(v5->actions), v6->vfptr->type(v6) != 12) )
    {
        ...
        if( !v9 )
            return 0;
        RGE_Action_List::delete_list(v5->actions);
        (*(void (**)(RGE_Combat_Object *, RGE_Action *))&v5->vfptr[1].gap4[28])(v5, v9);
        ((void (*)(RGE_Action *, RGE_Combat_Object *, _DWORD, _DWORD, _DWORD))v9->vfptr->work)(
            v9,
            v5,
            LODWORD(tX),
            LODWORD(tY),
    ...
    else
    {
        ...
        ((void (*)(RGE_Action *, RGE_Combat_Object *, _DWORD, _DWORD, _DWORD))v7->vfptr->work)(
            v7,
            0,
            LODWORD(tX),
            LODWORD(tY),
...
//----- (00431340) --------------------------------------------------------
int RGE_Combat_Object::unload(int unconditional, float x, float y, float z)
{
    RGE_Combat_Object *v5; // esi@1
    RGE_Action *v6; // eax@2
    RGE_Action *v7; // eax@3
        ...
        || (v6 = RGE_Action_List::get_action(v5->actions), v6->vfptr->type(v6) != 12) )
    {
        ...
        if( !v10 )
            return 0;
        RGE_Action_List::delete_list(v5->actions);
        (*(void (**)(RGE_Combat_Object *, RGE_Action *))&v5->vfptr[1].gap4[28])(v5, v10);
        ((void (*)(RGE_Action *, RGE_Combat_Object *, _DWORD, _DWORD, _DWORD))v10->vfptr->work)(
            v10,
            v5,
            LODWORD(x),
            LODWORD(y),
...
int RGE_Combat_Object::stopAction()
{
    ...
    (*(void (**)(RGE_Combat_Object *, RGE_Sprite *))&v1->vfptr->gap4[52])(v1, v1->master_obj->sprite);
    return 1;
}
...
//----- (00431490) --------------------------------------------------------
double RGE_Combat_Object::rateOfFire()
{
    return *(float *)&this->master_obj[1].movement_type;
}

//----- (004314A0) --------------------------------------------------------
double RGE_Combat_Object::damageCapability()
{
...
//----- (004314F0) --------------------------------------------------------
double RGE_Combat_Object::damageCapability(RGE_Static_Object *attackingUnit)
{
    ...
    *(float *)&attackingUnita = result;
    ((void (*)(RGE_Combat_Object *, _DWORD, _DWORD, RGE_Static_Object *, RGE_Player *, RGE_Static_Object *))v4->vfptr->calculateDamage)(
        v4,
...
//----- (00431530) --------------------------------------------------------
BOOL RGE_Combat_Object::inAttackRange(RGE_Static_Object *target)
{
    RGE_Combat_Object *v2; // esi@1
    RGE_Master_Static_Object *v3; // eax@1
    double v4; // st7@1
...
//----- (004315A0) --------------------------------------------------------
double RGE_Combat_Object::weaponRange()
{
    return this->master_obj[1].construction_radius_x;
}

//----- (004315B0) --------------------------------------------------------
double RGE_Combat_Object::minimumWeaponRange()
{
...
//----- (00431610) --------------------------------------------------------
double RGE_Combat_Object::currentTargetX()
{
    RGE_Combat_Object *v1; // esi@1
    RGE_Action *v2; // eax@2
    double result; // st7@2
...
//----- (00431640) --------------------------------------------------------
double RGE_Combat_Object::currentTargetY()
{
    RGE_Combat_Object *v1; // esi@1
    RGE_Action *v2; // eax@2
    double result; // st7@2
...
//----- (00431670) --------------------------------------------------------
double RGE_Combat_Object::currentTargetZ()
{
    RGE_Combat_Object *v1; // esi@1
    RGE_Action *v2; // eax@2
    double result; // st7@2
...
//----- (004316A0) --------------------------------------------------------
double RGE_Combat_Object::teleport(float x, float y, float z)
{
    RGE_Combat_Object *v4; // esi@1
    int v5; // edi@1
    int v6; // ebp@1
