 ...
 * @version 1.0
 */
//typedef int __stdcall TButtonPanel::DrawType(HDC, LPCSTR, int, LPRECT, UINT);
...
public:
    ...
    signed int setup(TDrawArea *render_area_in, TPanel *parent_panel_in, int x_in, int y_in, int wid_in, int hgt_in, TPanel *a8, TDigital *sound_in, TPanel *notifyTypeIn, int id_in);
    TButtonPanel::ButtonType buttonType();
    int drawType();
    TButtonPanel::NotifyType notifyType();
    int setDrawType(int draw_type);
    void setButtonType(TButtonPanel::ButtonType buttonTypeIn);
