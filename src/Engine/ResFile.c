...
static void RESFILE_open_new_resource_file(char *resFileName, char *password, char *path, int open_mode)
{
  HANDLE v4; // eax@2
  void *v5; // esi@2
  HANDLE v6; // ebx@4
...
static void RESFILE_close_new_resource_file(char *res_file)
{
  ResFileHdr *v1; // edi@1
  ResFileHdr *v2; // ebx@1
...
static bool RESFILE_locate_resource(unsigned int rType, unsigned int rId, int *file, int *offset, char **mapped_file, int *size)
{
  ResFileHdr *v6; // esi@1
...
static char * RESFILE_load(unsigned int rType, unsigned int rId, int *rLoadType, int *rDataSize)
{
...
static char * RESFILE_Extract_to_File(unsigned int rType, unsigned int rId, char *file_name, _iobuf **file)
{
  char *result; // eax@1
  char *v5; // ebp@1
  _iobuf *v6; // eax@5
...
static void RESFILE_Set_Missing_Flag(int FlagVal)
{
  resource_missing_flag = FlagVal;
}

static bool RESFILE_Decommit_Mapped_Memory(char *ResData, int resSize)
{
...
static bool RESFILE_build_res_file(char *build_list_file, char *source_path, char *target_path)
{
  FILE *v3; // eax@1
  FILE *v4; // esi@1
  int result; // eax@2
...
static int BUILDRES_get_files_resource_type(char *filename)
{
  char *v1; // edx@1
  int v2; // ecx@1
